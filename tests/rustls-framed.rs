...
#[test_async]
async fn test_rustls_framed() -> Result<(), IoError> {
    test_tls(
...
async fn test_tls(acceptor: TlsAcceptor, connector: TlsConnector) -> Result<(), IoError> {
    ...
    let server_ft = async {
        ...
        for i in 0..ITER {
            ...
            let frame_bytes = framed.next().await.expect("frame").expect("values");
            debug!("server: loop {}, received reply back bytes: {}", i, frame_bytes.len());
            let slice = frame_bytes.as_ref();
